...
| RF-506, RF-309 · ficha de referencia | La ficha de una referencia con su bloque «Obras citadas», que es **la referencia leída por el otro lado** y no existía en ningún sitio: qué obras la citan y en qué página de ella sale cada una. Cubierto en `referenceRecord.test.ts` (25 casos): el orden por identificador de catalogación y **no por páginas** —`pages` es texto libre (RF-504) y ordenar por eso pondría «lám. XII» antes que «p. 9»—, la abreviatura de la página en sus tres formas, la página sin anotar que **no es «s/p»**, la obra en la papelera marcada y no escondida, la obra que no se puede leer dejando la fila puesta y sin enlace —tirarla acortaría en silencio la lista de obras que la citan—, las citas retiradas fuera porque su propia obra ya no las cuenta, y el aviso del bloque vacío diciendo que **no es un dato pendiente**. Y **comprobado en Chromium a 390×844 el 5 de agosto de 2026**, 17 comprobaciones sobre el cuerpo real: el orden pintado, el enlace que lleva de verdad a la ficha de obra, la fila no enlazada de la obra ilegible, «Sin dato» donde no hay dato y el bloque vacío. Sin miniatura, que lo pide el requisito. **Lo que sigue sin hacerse desde ninguna pantalla** es elegir qué referencia es el catálogo de una exposición (RF-503) y dar una de alta sin citarla | **Hecho** |
