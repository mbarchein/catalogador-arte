...
describe('los tamaños se escriben como se leen', () => {
  it('en múltiplos de 1000, que es lo que enseña el panel del servicio', () => {
    // Con 1024 saldría «976,6 KB» y no cuadraría con Supabase ni con Backblaze.
    // Una cifra que discrepa del panel oficial no se cree, y con razón.
...
describe('cuánto queda', () => {
  ...
  it('pasarse del límite no es espacio de sobra', () => {
    // Sin el suelo en cero saldría «quedan -2 GB libres», que además de absurdo
    // pinta la barra al revés.
...
describe('el aviso sale cuando queda poco, y no antes', () => {
  ...
  it('y lleno dice qué va a pasar y qué se puede hacer', () => {
    // «Lleno» sin salida es una mala noticia y nada más. Y lo que se puede hacer
    // no es borrar, que en este proyecto no existe (RF-901).
...
describe('lo que matiza la cifra se dice', () => {
  ...
  it('cero ficheros no se lee como una avería', () => {
    ...
    // Cuatro cifras van sin punto en español y cinco con él: es la regla de la
    // lengua, no un descuido del formato.
