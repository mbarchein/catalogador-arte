...
| RF-106 | Al Lector no se le muestra ningún control de escritura: ni «Editar», ni «+ Nueva…», ni acceso a la papelera. **Y no se le muestran a destiempo:** el papel llega en una consulta que va detrás de la sesión, así que el menú de abajo abría con tres pestañas y se convertía en cinco un momento después. Mientras esa respuesta está en el aire vale **el papel que el dispositivo recuerda de ese mismo usuario**, y en cuanto llega manda la respuesta. Lo recordado solo ofrece y nunca niega: a un lector recordado se le espera en vez de echarlo, porque negar de memoria expulsaría de la pantalla a quien acaban de nombrar Catalogador. No es un permiso ni podría serlo —la clave anónima viaja en el cliente y quien decide son las políticas RLS—, y se borra al cerrar sesión, que el teléfono se comparte. |
...
| RF-518 | Una exposición puede llevar **el cartel** de la muestra, y su **miniatura se ve en el listado**: una lista de títulos y años se recorre leyendo, y el cartel es lo que la hace reconocible de un vistazo. Se guardan **dos ficheros y no tres** —la miniatura de 400 px que pinta el listado y la copia de consulta de 2000 px que se abre al tocarla—: el original de una fotografía de obra es el documento de conservación del que sale la copia para una imprenta (RF-420) y un cartel no es eso, y subir los megabytes de la foto de un móvil tres veces desde un almacén es lo que se evita. Las dos rutas y la fecha se escriben **juntas o ninguna**. Quitarlo deja el fichero en el almacén: aquí nunca se borra nada (RF-901). En el listado, la miniatura se pinta con **la proporción de un cartel** (1:√2, la de la serie A y casi la de un 50×70) y no cuadrada: un cuadrado le corta la cabecera y el pie, que es donde están el título y las fechas. Sin cartel, el mismo hueco con un recuadro que lo dice, nunca un vacío (RF-304). |
| RF-519 | La búsqueda del listado de exposiciones entiende un año como **un año en el que la muestra estuvo abierta**, y no como cuatro cifras dentro de su texto. `year` está clavado a la apertura, así que una muestra de 2010 a 2025 no aparecía al teclear 2016 aunque la fila imprima los dos extremos: el filtro contestaba «no se han encontrado exposiciones» sobre una muestra cuyas fechas dicen lo contrario. Es el mismo solapamiento que RF-602 pide para el rango de fechas de las obras. El cierre sale de `end_date` y **nunca de la nota de fechas**, que es prosa: leer un año de ahí sería inventar una fecha que la catalogadora no ha escrito. El año se compara contra el tramo y el resto de lo teclado contra el texto —«2016 Cáceres» es lo que estuvo abierto en 2016 en Cáceres—, y un número de cuatro cifras que no cabe en el rango de un año sigue buscándose como texto, que «1000 días» es un título. |
...
| RF-1105 | **Revisado por RF-1107, y hoy dice lo contrario de lo que se construyó.** Pedía que la gestión de usuarios se hiciera desde el panel de Supabase y que la aplicación no tuviera pantallas de administración de usuarios. Con tres personas era razonable; lo que lo dejó de ser es que RF-108 estaba a medias: el trigger que prohíbe cambiar un rol sin ser Superusuario existía desde el primer día, pero la única política de escritura de perfiles era la de la fila propia, así que **nadie podía asignar un rol desde la aplicación aunque el requisito lo diera por hecho**. La gestión vive ahora en la aplicación (RF-1107); del panel solo queda lo que por fuerza se hace fuera — nombrar al primer Superusuario y recuperar el catálogo si se queda sin ninguno. |
...
| RF-1107 | **La gestión del equipo está en la aplicación y es solo del Superusuario** (revisa RF-1105, completa RF-108). Una pantalla propia, colgada de «Tablas» y comprobada dentro además de en la fila que lleva a ella. Hace tres cosas: **invitar** por correo —lo único que exige la clave de servicio, y por eso lo único que vive en una función Edge (RF-111); se nace Lector y los permisos se dan después—, **asignar rol** y **quitar o devolver el acceso**. El acceso es un estado del perfil que `can_read()`, `can_edit()` y `my_role()` exigen, de modo que retirarlo cierra el catálogo entero de una vez en lugar de repetir la regla en veintitantas políticas; y **no es un borrado** (RF-901): la cuenta no se borra nunca, porque el perfil cae en cascada desde ella y el catálogo se quedaría firmado por un identificador que ya no existe. Quien se queda sin acceso lee siempre su propio perfil —única excepción de la política de lectura— para que la aplicación pueda decírselo en vez de enseñarle pantallas vacías. Y la base **rechaza degradar o dejar sin acceso al último Superusuario activo**: sin ninguno, la asignación de roles solo se recupera desde el panel, y un clic no puede tener esa consecuencia. |
| RF-1108 | **Un cambio de rol o de acceso deja traza**: quién, a quién, de qué a qué y cuándo. En tabla propia y no en el registro de cambios del catálogo (RF-1501), que exige `catalog_id` y tiene otra regla de visibilidad — su propio comentario ya decía que un cambio de rol es probablemente el cambio más sensible del sistema y que la decisión era del propietario. La escribe un trigger, solo se añade —ni el dueño de la tabla ni la clave de servicio pueden reescribirla— y solo la lee el Superusuario: habla de personas, no del catálogo. Guarda el correo del sujeto además de su identificador, porque la cuenta puede borrarse desde el panel y la traza tiene que seguir diciendo de quién habla. |
...
| RF-1619 | El dossier se organiza en **secciones**: un rótulo, una entradilla opcional, y **los elementos que dicen pertenecerle**. La pertenencia es un dato de cada elemento y no su posición, para que **una obra pueda ir suelta en cualquier sitio** —también detrás de una sección— y para que mover no sea cambiar de sección. Meter una obra en un bloque o sacarla se pide en su fila. Lo que va suelto sale en el PDF sin rótulo, y la pantalla lo avisa. Los elementos de una sección van **seguidos, detrás de su rótulo**: la portadilla anuncia un bloque y el índice dice en qué página empieza, así que un bloque repartido mentiría impreso. El orden y la pertenencia se guardan **juntos o ninguno**. |
| RF-1620 | Una sección **se mueve entera, con sus obras dentro**, que es el trabajo que si no son diez toques, y **sin cambiar la pertenencia de nada**: se cambia con el bloque de al lado, donde una obra suelta es un bloque de una, así que se desliza entre las sueltas de una en una. Un elemento de dentro se mueve entre los de su sección; uno suelto salta el bloque de al lado de un toque. En el PDF, el rótulo de la sección va en el pie de todas sus páginas: una hoja impresa que se separa del resto sigue diciendo de qué bloque es. |
| RF-1621 | Cada sección decide si su rótulo **se lleva una página propia**. Con la maqueta de una obra por página, «empezar en página nueva» no significaría nada —toda obra ya empieza en una—, así que la decisión es si se gasta una hoja en anunciar el bloque. |
| RF-1622 | El dossier puede llevar un **índice de secciones** detrás de la portada, con las obras que lleva cada una y la página en la que empieza. O están todas o no hay índice: uno que se salta secciones miente. El número de página **se mide, no se estima**: se escribe cuando las hojas ya están escritas, contando también las del propio índice. |
| RF-1623 | Las secciones se pueden **crear solas agrupando por serie**, de una vez: crea los rótulos que falten, coloca cada obra bajo la suya y deja todo editable. **No es un criterio vivo** — si lo fuera, dar de alta una obra la metería en un documento ya mandado—. Las que no tienen serie van a una sección nombrada y al final, y los rótulos escritos a mano se conservan. |
| RF-1624 | La biografía, el currículum, los párrafos del dossier y su portada admiten **marcado ligero**: títulos, listas, negrita y cursiva, y nada más. Está para lo que hay: **pegar un texto de una página web y conservar su forma** — al pegar, el HTML del portapapeles se traduce a marcas, y lo que no está en la lista (tipografías, colores, imágenes, enlaces) se cae; una tabla se lee como líneas con las celdas separadas. **Lo que se guarda es texto**, nunca HTML: no hay servidor y las políticas RLS son todo el perímetro, así que un HTML guardado se ejecutaría en la sesión de quien lo abriera. Lo escrito sin marcas sigue significando lo mismo, y un asterisco suelto es un asterisco. **En el móvil el evento de pegar da solo texto plano** —medido—, así que hay un botón «Pegar con formato» que lee el portapapeles del sistema con la API asíncrona, que es la única vía al HTML en un teléfono y exige un toque explícito. Cuando ni por ahí hay formato, el portapapeles del sistema no lo tiene: se dice en una línea y las marcas se ponen con los botones. |
| RF-1625 | **Un texto largo ocupa las hojas que necesite.** Una biografía o un currículum que no caben en una hoja siguen en la siguiente, con su pie y su sección; lo que no cabía nunca se dibuja por debajo del margen, que es perder texto sin avisar. El recuento del pie y el número del índice se escriben al final, contando hojas de verdad. |
...
| Esquema: dossier, sus obras ordenadas y sus emisiones (RF-1600) | **Construido** el 11 de agosto de 2026 ([ADR-011](decisiones/ADR-011-el-dossier.md)), con sus políticas y sus veinticuatro comprobaciones |
| Frontend: armar un dossier y emitir su PDF (RF-1600) | **Construido** el 11 de agosto de 2026: listado con buscador, alta, la pantalla de armarlo —obras, secciones, textos y biografía, con miniatura, movimiento por elemento y por sección entera, plegado, precios y notas, y agrupar por serie— y el generador del PDF, con una obra por página, portada, portadillas, índice y emisiones descargables. Puerta en «Tablas». Falta el recorrido con una obra real y mirar el papel |
