...
| RNF-106 · el «atrás» sobre un modal | **La primera fila de RNF-106 con asertos**, y los tiene porque lo que hay debajo no es maquetación: es el historial del navegador, estado global compartido con el enrutador, y los cuatro fallos posibles no se ven leyendo el código. Cubierto en `useCloseOnBack.test.tsx`: abrir empuja una entrada sin cambiar de dirección; el «atrás» cierra el modal y no la pantalla; con dos anidados cierra solo el de encima y hace falta un segundo toque para el de debajo; cerrar por la ✕ consume la entrada, para que el «atrás» siguiente no parezca averiado; un modal que se niega a cerrarse —la hoja que sube un documento— recupera su entrada y el «atrás» no saca de la pantalla; y navegar desde dentro del modal no hace que cerrarlo abandone la pantalla nueva. Que la hoja compartida por catorce pantallas lo usa se comprueba en `ui.test.tsx`. **Queda para un teléfono de verdad** el gesto de arrastre desde el borde, que es como se va atrás en Android e iOS sin botón | **Hecho** |
