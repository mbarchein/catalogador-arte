...
| RF-1619 | El dossier se organiza en **secciones**: un rótulo, una entradilla opcional, y **todo lo que viene detrás hasta la siguiente**. La pertenencia es la posición y no un campo, así que mover un rótulo por encima de una obra cambia de sección esa obra. Lo que va antes de la primera sección sale sin rótulo, y la pantalla lo avisa. |
| RF-1620 | Una sección **se mueve entera, con sus obras dentro**, que es el trabajo que si no son diez toques. Se cambia con **el bloque de al lado**, y lo que va suelto al principio del dossier cuenta como bloque: contando solo secciones, la única de un dossier no se podría mover a ningún sitio. Subir por encima de lo suelto deja esas obras **dentro** de la sección —la pertenencia es la posición (RF-1619)— y se avisa antes, porque deshacerlo cuesta un toque por obra. En el PDF, su rótulo va en el pie de todas sus páginas: una hoja impresa que se separa del resto sigue diciendo de qué bloque es. |
